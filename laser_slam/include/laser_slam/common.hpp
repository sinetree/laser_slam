...
typedef PointMatcher<float> PointMatcher1;
typedef typename PointMatcher1::DataPoints DataPoints;
...
static void correctTransformationMatrix(
    PointMatcher1::TransformationParameters* transformation_matrix) {
  CHECK_NOTNULL(transformation_matrix);

  PointMatcher1::Transformation* rigid_transformation =
      PointMatcher1::get().REG(Transformation).create("RigidTransformation");
...
static SE3 convertTransformationMatrixToSE3(
    const PointMatcher1::TransformationParameters& transformation_matrix) {
