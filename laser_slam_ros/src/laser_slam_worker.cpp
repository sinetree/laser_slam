...
Pose LaserSlamWorker::navOdometryToPose(const nav_msgs::Odometry::ConstPtr& odom_transform) {
  // Register new pose.
  Pose pose;
  SE3::Position pos(odom_transform->pose.pose.position.x, odom_transform->pose.pose.position.y,
                    odom_transform->pose.pose.position.z);
  SE3::Rotation::Implementation rot(odom_transform->pose.pose.orientation.w,
                                    odom_transform->pose.pose.orientation.x,
                                    odom_transform->pose.pose.orientation.y,
                                    odom_transform->pose.pose.orientation.z);
  pose.T_w = SE3(pos, rot);
  pose.time_ns = navTimeToCurveTime(odom_transform->header.stamp.toNSec());
...
Time LaserSlamWorker::navTimeToCurveTime(const Time& timestamp_ns) {
  if (!base_navtime_set_) {
    base_navtime_ns_ = timestamp_ns;
    base_navtime_set_ = true;
  }
  return timestamp_ns - base_navtime_ns_;
}
