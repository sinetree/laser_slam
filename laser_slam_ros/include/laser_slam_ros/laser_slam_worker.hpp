...
class LaserSlamWorker {
 ...
 public:
  ...
  laser_slam::Pose navOdometryToPose(const nav_msgs::Odometry::ConstPtr& odom_transform);
   std::shared_ptr<laser_slam::LaserTrack> laser_track_;
   laser_slam::Time rosTimeToCurveTime(const laser_slam::Time& timestamp_ns);
   laser_slam::Time navTimeToCurveTime(const laser_slam::Time& timestamp_ns);
